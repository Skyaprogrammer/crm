...
<script type="text/javascript">
	...
	function clickFn(obj){
		$("#cust_id").val($(obj).html());
		$("#showDiv").css("display","none");
	}
	
	function searchCid(obj){
		//应该一点击就会出所有的客户名称供我们选择，或者我们选择
		//这里一点击就会显示所有的客户名称
		var content="";
		$.post(
			"${pageContext.request.contextPath }/searchCidServlet",
			function(data){
				//已经获得了json数据
				if(data.length>0){
					for(var i=0;i<data.length;i++){
						content+="<div name="+data[i].cust_id+" style='padding:5px;cursor:pointer' onclick='clickFn(this)' onmouseover='overFn(this)' onmouseout='outFn(this)'>"+data[i].cust_name+"</div>";
					}
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						...
						<TABLE cellSpacing=0 cellPadding=5  border=0>
							<tr>
								...
								<td colspan="3">
								<div>
								<input type="text" name="cust_id" style="WIDTH: 180px" onclick="searchCid(this)" id="cust_id"/>
								</div>
